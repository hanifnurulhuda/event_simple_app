    ...
    <section class="mx-auto max-w-3xl px-4 pb-12">
      <div class="card">
        ...
        <form v-if="!participant" class="mt-6 grid gap-4" @submit.prevent="loadParticipant">
          ...
          <button class="btn-primary" :disabled="loading || !validEventCode">Cari Peserta</button>
        </form>
...
const validEventCode = computed(() => isValidEventCode(String(route.params.event_code || ''), 'survey'))

const { data: questions } = await useAsyncData('survey-questions', () =>
  $fetch<SurveyQuestion[]>('/api/survey-questions')
)

onMounted(() => {
  if (!validEventCode.value) {
    tone.value = 'error'
    message.value = 'QR survey tidak valid. Gunakan QR resmi dari panitia.'
  }
})
...
const submitSurvey = async () => {
  ...
  try {
    await $fetch('/api/survey-responses', {
      method: 'POST',
      body: { participant_id: participant.value.id, ...answers }
    })
    const status = certificateStatusFor({ ...participant.value, survey_submitted: true })
    await $fetch(`/api/participants/${participant.value.id}`, {
      method: 'PATCH',
      body: { survey_submitted: true, certificate_status: status }
    })
    tone.value = 'success'
