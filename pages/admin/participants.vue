    ...
    <section class="mx-auto max-w-7xl px-4 py-8">
      <div class="card">
        ...
        <div class="mt-5 grid gap-3 md:grid-cols-5">
           <input v-model="search" class="input md:col-span-2" :placeholder="searchPlaceholder" />
         ...
         <div class="mt-5 overflow-x-auto"><table class="min-w-full text-left text-sm"><thead class="text-slate-500"><tr><th class="p-2">Kode</th><th class="p-2">Nama</th><th class="p-2">Kelas</th><th v-if="showSchool" class="p-2">Sekolah</th><th class="p-2">WA</th><th class="p-2">Hari</th><th class="p-2">Hadir</th><th class="p-2">Survey</th><th class="p-2">Action</th><th class="p-2">Sertifikat</th></tr></thead><tbody><tr v-for="p in filtered" :key="p.id" class="border-t"><td class="p-2 font-bold">{{ p.participant_code }}</td><td class="p-2">{{ p.name }}</td><td class="p-2">{{ p.class_name || '-' }}</td><td v-if="showSchool" class="p-2">{{ p.school }}</td><td class="p-2">{{ p.whatsapp }}</td><td class="p-2">{{ p.event_day }}</td><td class="p-2">{{ p.attended ? 'Ya' : 'Belum' }}</td><td class="p-2">{{ p.survey_submitted ? 'Ya' : 'Belum' }}</td><td class="p-2">{{ p.action_plan_submitted ? 'Ya' : 'Belum' }}</td>        <td class="p-2">{{ certificateLabel(certificateStatusFor(p)) }}</td></tr></tbody></table></div>
      </div>
    </section>
  </main>
</template>
...
const searchPlaceholder = computed(() => showSchool.value ? 'Cari nama, sekolah, WA, kode' : 'Cari nama, WA, kode')

const filtered = computed(() => participants.value.filter((p) => {
  const parts = [p.name, p.class_name, p.whatsapp, p.participant_code]
  if (showSchool.value) parts.push(p.school)
