  ...
  <main class="min-h-screen bg-slate-50">
    ...
    <section class="mx-auto max-w-7xl px-4 py-8">
      <div class="card">
        ...
        <div class="mt-5 overflow-x-auto"><table class="min-w-full text-left text-sm"><thead class="text-slate-500"><tr><th class="p-2">Kode</th><th class="p-2">Nama</th><th class="p-2">Kelas</th><th class="p-2">Sekolah / Asal</th><th class="p-2">Hadir</th><th class="p-2">Survey</th><th class="p-2">Status</th><th class="p-2">Dibuka</th></tr></thead><tbody><tr v-for="p in filtered" :key="p.id" class="border-t"><td class="p-2 font-bold">{{ p.participant_code }}</td><td class="p-2">{{ p.name }}</td><td class="p-2">{{ p.class_name || '-' }}</td><td class="p-2">{{ p.school }}</td><td class="p-2">{{ p.attended ? 'Ya' : 'Belum' }}</td><td class="p-2">{{ p.survey_submitted ? 'Ya' : 'Belum' }}</td>        <td class="p-2">{{ certificateLabel(certificateStatusFor(p)) }}</td><td class="p-2">{{ p.certificate_viewed_at || '-' }}</td></tr></tbody></table></div>
        </template>
      </div>
    </section>
  </main>
