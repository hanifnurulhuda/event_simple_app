  ...
  <main class="min-h-screen bg-slate-50">
    ...
    <section class="mx-auto max-w-7xl px-4 py-8">
      <div class="flex flex-wrap items-end justify-between gap-4">
        ...
        <select v-model="dayFilter" class="input max-w-48"><option value="all">Semua hari</option><option>17 Juni</option><option>18 Juni</option></select>
      </div>
      ...
      <div class="mt-6 grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
        ...
        <div class="card overflow-x-auto">
          ...
          <table class="mt-4 min-w-full text-left text-sm">
            <thead class="text-slate-500"><tr><th class="p-2">Nama</th><th class="p-2">Kelas</th><th v-if="showSchool" class="p-2">Sekolah</th><th class="p-2">Hari</th><th class="p-2">Hadir</th><th class="p-2">Survey</th><th class="p-2">Action Plan</th></tr></thead>
            <tbody><tr v-for="p in filtered" :key="p.id" class="border-t"><td class="p-2 font-bold">{{ p.name }}</td><td class="p-2">{{ p.class_name || '-' }}</td><td v-if="showSchool" class="p-2">{{ p.school }}</td><td class="p-2">{{ p.event_day }}</td><td class="p-2">{{ p.attended ? 'Ya' : 'Belum' }}</td><td class="p-2">{{ p.survey_submitted ? 'Ya' : 'Belum' }}</td><td class="p-2">{{ p.action_plan_submitted ? 'Ya' : 'Belum' }}</td></tr></tbody>
          </table>
        </div>
      </div>
    </section>
